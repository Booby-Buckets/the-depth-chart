...
(function (g) {
  ...
  function loadEff() { return _effP || (_effP = fetch('scripts/data/team_pace_eff.json?v=3').then(r => r.ok ? r.json() : null).then(j => (_eff = j)).catch(() => null)); }
  function loadProj() { return _projP || (_projP = fetch('scripts/data/stat_overall_projected.json?v=43').then(r => r.ok ? r.json() : null).then(j => (_proj = j)).catch(() => null)); }
