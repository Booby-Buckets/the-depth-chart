...
SEASON = 2026
VMAX = 2.5           # bump ceiling — reachable, but only by the genuinely all-around
RAW_FULL = 3.6       # raw score that earns the full ceiling (set high so 2.5 is hard)
GAMMA = 2.1          # convex curve: mid-tier players get little; only the top climb
...
def main():
    ...
    for r in recs:
        ...
        raw = ((breadth - 1.5) * 0.9 + bpm_term) * role * taper
        norm = max(0.0, min(1.0, raw / RAW_FULL))     # 0 at/under baseline, 1 at RAW_FULL
        bump = VMAX * (norm ** GAMMA)                 # convex: hard to approach the ceiling
